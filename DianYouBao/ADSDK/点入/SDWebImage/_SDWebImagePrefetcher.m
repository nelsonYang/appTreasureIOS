...
#import "_SDWebImagePrefetcher.h"
#import "_SDWebImageManager.h"

@interface _SDWebImagePrefetcher ()
@property (nonatomic, retain) NSArray *prefetchURLs;
@end

@implementation _SDWebImagePrefetcher

static _SDWebImagePrefetcher *instance;

@synthesize prefetchURLs;
@synthesize maxConcurrentDownloads;

+ (_SDWebImagePrefetcher *)sharedImagePrefetcher
{
    if (instance == nil)
    {
        instance = [[_SDWebImagePrefetcher alloc] init];
...
- (void)startPrefetchingAtIndex:(NSUInteger)index withManager:(_SDWebImageManager *)imageManager
{
...
- (void)reportStatus
{
    ...
    ////WLLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    ////WLLog(@"Prefetched %d out of %d", _finishedCount, [self.prefetchURLs count]);
...
{
    _finishedCount++;
    ////WLLog(@"Prefetched %d out of %d (Failed)", _finishedCount, [self.prefetchURLs count]);
