...
- (id)initWithFrame:(CGRect)frame arrowImageName:(NSString *)arrow textColor:(UIColor *)textColor  {
    if((self = [super initWithFrame:frame])) {
        ...
        _arrowImage =[[UIImageView alloc] initWithFrame:CGRectMake(0, 0,64,64)];
        _arrowImage.image = [UIImage imageNamed:@"update_pic_001"];
        _arrowImage.animationImages = [NSArray arrayWithObjects:[UIImage imageNamed:@"update_pic_001"],[UIImage imageNamed:@"update_pic_002"], nil];
        //设置动画总时间
        _arrowImage.animationDuration=0.2;
        //设置重复次数，0表示不重复
...
-(void)layoutSubviews
{
    ...
    _arrowImage.frame = CGRectMake(25.0f, self.frame.size.height - 65.0f, 64.0f, 64.0f);
    _activityView.frame = CGRectMake(245.0f, self.frame.size.height - 38.0f, 20.0f, 20.0f);
}
#pragma mark -
#pragma mark Setters
	...
	if (_state != state) {
        ...
        switch (state) {
            case PullRefreshPulling:
                ...
                _statusLabel.text = NSLocalizedString(@"松开即可加载更多内容...", @"松开即可加载更多内容...");
                [CATransaction begin];
...
- (void)RefreshScrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.isDragging)
    {
        ...
        if (!self.refreshHeaderView.hidden)
        {
            ...
            if (self.refreshHeaderView.state == PullRefreshPulling&&scrollView.contentOffset.y > -50.0f && scrollView.contentOffset.y < 0.0f && !_loading) {
                self.refreshHeaderView.state = PullRefreshNormal;
            } else if (self.refreshHeaderView.state == PullRefreshNormal && scrollView.contentOffset.y < -50.0f && !_loading) {
        ...
        if (!self.refreshFooterView.hidden)
        {
...
- (void)RefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	else if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHeight && !_loading)
    {
        if (!self.refreshFooterView.hidden)
        {
...
//当开发者新数据加载完毕调用此方法，[delegate RefreshScrollViewDataSourceDidFinishedLoading: scrollView];
